...
function createContentHierarchyPlugin (opts) {
  ...
  const filter = typeof options.filter === 'string' ? minimatch.filter(options.filter) : options.filter
  ...
  return function createContentHierarchy (files, metalsmith, done) {
    function getParents (child, parentPath) {
      ...
      var parent = metalsmith.metadata().contentfulIDMap[child.parent.data.sys.id]
